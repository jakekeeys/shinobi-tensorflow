$(document).ready(function(e){
    ...
    $.timelapseJpeg.getLiveStream = function(){
        ...
        $.timelapseJpeg.liveStreamView.html(`<iframe src="${$.ccio.init('location',$user)+'/'+$user.auth_token+'/embed/'+$user.ke+'/'+selectedMonitor+'/jquery|fullscreen'}"></iframe>`)
    ...
    $.timelapseJpeg.e.on('click','.download_mp4',function(){
        ...
        var runDownloader = function(){
            ...
            var selectedMonitor = $.timelapseJpeg.monitors.val()
            var generatorUrl = $.ccio.init('location',$user) + '/' + $user.auth_token + '/timelapse/' + $user.ke + '/' + selectedMonitor
